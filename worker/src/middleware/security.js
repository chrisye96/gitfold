 ...
 */
export async function validateUrl(c, next) {
 ...
 */
export async function resolveTier(c, next) {
 ...
 */
export function checkLimits(fileCount, totalBytes, tierFileLimit) {
    const effectiveLimit = tierFileLimit ?? LIMITS.maxFiles;
    if (fileCount > effectiveLimit) {
        return {
            ok: false,
            response: errorResponse(413, 'TOO_MANY_FILES', `Directory contains ${fileCount} files (limit: ${effectiveLimit}).`, effectiveLimit < LIMITS.maxFiles
