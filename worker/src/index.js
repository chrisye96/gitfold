...
// ─── Health check ─────────────────────────────────────────────────────────────
app.get('/health', (c) => Response.json({
    ok: true,
    service: 'worker',
    version: '1.0.0',
...
// ─── API docs redirect ────────────────────────────────────────────────────────
app.get('/docs', (c) => c.redirect('https://gitfold.cc/docs', 302));
...
async function downloadResultHandler(c) {
    ...
    if (!jobId) {
        return Response.json({ code: 'MISSING_PARAM', message: 'Missing jobId query parameter.' }, { status: 400, headers: corsHeaders(c.req.header('Origin')) });
    }
    ...
    if (!zipData) {
        return Response.json({ code: 'JOB_EXPIRED', message: 'Job not found or expired (5-minute window).' }, { status: 404, headers: corsHeaders(c.req.header('Origin')) });
    }
