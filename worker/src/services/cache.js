 ...
 */
export function zipCacheKey(info, commitSha) {
 ...
 */
export async function getZipFromR2(env, key) {
    if (!env.R2_CACHE)
        return null;
    try {
 ...
 */
export async function saveZipToR2(env, key, data) {
    if (!env.R2_CACHE)
        return;
    try {
 ...
 */
export async function fetchCommitSha(info, token) {
    const headers = {
    ...
    try {
        const res = await fetch(`${GITHUB_API}/repos/${info.owner}/${info.repo}/commits/${encodeURIComponent(info.branch)}`, { headers });
        if (!res.ok)
            return null;
 ...
 */
export async function cleanupOldZips(env, maxAgeDays = 30) {
    if (!env.R2_CACHE)
        return { deleted: 0 };
