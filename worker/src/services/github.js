...
async function checkGitHubResponse(res, context = '') {
    ...
    if (res.status === 404) {
        throw errorResponse(404, 'NOT_FOUND', `Not found${context ? ': ' + context : ''}.`, 'Check that the repository, branch, and path exist.');
    }
    let msg = '';
    try {
 ...
 */
export async function fetchTree(info, token, kv) {
    ...
    const branchRes = await fetch(`${GITHUB_API}/repos/${info.owner}/${info.repo}/branches/${encodeURIComponent(info.branch)}`, { headers });
    ...
    const treeRes = await fetch(`${GITHUB_API}/repos/${info.owner}/${info.repo}/git/trees/${treeSha}?recursive=1`, { headers });
    ...
    const entries = treeData.tree
        ...
        .map(e => ({ path: e.path, type: 'blob', size: e.size ?? 0, sha: e.sha }));
    if (entries.length === 0) {
        throw errorResponse(404, 'NOT_FOUND', `No files found in '${info.path || '/'}'. Check that the path exists.`, 'Check that the directory exists and contains files.');
    }
 ...
 */
export async function fetchAllFiles(entries, info) {
    const BATCH = 8;
...
export function buildInfo(entries, info) {
    return {
