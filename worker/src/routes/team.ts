...
function requireSession(sessionUser?: SessionUser) {
  if (!sessionUser) {
...
function requirePower(sessionUser?: SessionUser) {
...
async function getUserTeam(db: D1Database, userId: string) {
  // Check if owner
  ...
  const membership = await db
    ...
    `)
    .bind(userId)
    .first<{ id: string; name: string; owner_id: string; created_at: number; role: string }>()
...
team.post('/team/create', async (c) => {
  ...
  ).bind(teamId, name, sessionUser!.userId, now).run()

  return Response.json(
    { ok: true, team: { id: teamId, name, ownerId: sessionUser!.userId, createdAt: now } },
...
team.post('/team/invite', async (c) => {
  ...
  if (existing) {
    ...
    await c.env.DB
      .prepare(
        'INSERT INTO team_members (id, team_id, email, role, status, invite_token, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
      )
...
team.get('/team/members', async (c) => {
  ...
  const members = await c.env.DB
    ...
    .all<{
      ...
      joined_at: number | null; created_at: number
      github_login: string | null; avatar_url: string | null
    }>()
...
team.post('/team/join', async (c) => {
  ...
  const invite = await c.env.DB
    ...
    .bind(inviteToken)
    .first<{ id: string; team_id: string; email: string }>()
